...
export const importWorkLog = async (
  ...
  // CSVファイルを読み込む
  const csvContent = await window.electronAPI.readFile(csvPath, { encoding: 'utf8' });
  ...
  // プロジェクト名の一覧を取得
  const uniqueProjectNames = Array.from(new Set(data.map(row => row.project_name)));
  ...
  uniqueProjectNames.forEach(projectName => {
    if (!projectIdMap.has(projectName)) {
      const description = data.find(row => row.project_name === projectName)?.project_description || '';
