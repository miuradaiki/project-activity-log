...
export const restoreFromBackup = async (backupPath: string) => {
  try {
    // バックアップからデータを読み込み
    const projectsData = await window.electronAPI.readFile(`${backupPath}/projects.json`, { encoding: 'utf8' });
    const timeEntriesData = await window.electronAPI.readFile(`${backupPath}/timeEntries.json`, { encoding: 'utf8' });
