...
export const generateTestData = (existingProjects: Project[], existingTimeEntries: TimeEntry[]) => {
  console.log('generateTestData started', { existingProjects: existingProjects.length, existingTimeEntries: existingTimeEntries.length });
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const numEntries = isWeekend ? Math.floor(Math.random() * 2) : Math.floor(Math.random() * 4) + 1;
  ...
  console.log('generateTestData completed', { projects: result.projects.length, timeEntries: result.timeEntries.length });
  
  return result;
};

// テストデータのみを削除する関数（既存データは保持）
export const clearTestData = async (projects: Project[], timeEntries: TimeEntry[]) => {
  const testPrefix = '[TEST] ';
  
  // テストデータ以外のプロジェクトを保持
  const nonTestProjects = projects.filter(p => !p.name.startsWith(testPrefix));
  const nonTestProjectIds = new Set(nonTestProjects.map(p => p.id));
  
  // テストプロジェクトに関連しない時間エントリーを保持
  const nonTestTimeEntries = timeEntries.filter(t => nonTestProjectIds.has(t.projectId));
