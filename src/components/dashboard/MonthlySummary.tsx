...
import { Box, Typography, Select, MenuItem, FormControl, InputLabel, Tab, Tabs, useTheme } from '@mui/material';
...
export const MonthlySummary: React.FC<MonthlySummaryProps> = ({ projects, timeEntries }) => {
  ...
  const projectDistribution = useMemo(() => {
    ...
    return getProjectDistribution(timeEntries, projects, startOfMonth, endOfMonth);
      ...
      return (
        <Box sx={{ 
          ...
          <Typography variant="body2">
            {payload[0].value.toFixed(1)} {t('units.hours')} ({payload[0].payload.percentage}%)
          </Typography>
        </Box>
      );
    }
    ...
    <Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider', mt: 2, mb: 2 }}>
        ...
        >
          ...
          />
          <Tab 
            label={isEnglish ? "Weekly Trend" : "週別推移"} 
            value="weekly" 
          />
        </Tabs>
      </Box>
          ...
          {monthlyData.length > 0 ? (
            ...
            <Box sx={{ 
              ...
              <Typography variant="body1" color="text.secondary">
                {isEnglish ? 'No activity data for this month' : 'この月の稼働データがありません'}
              </Typography>
            </Box>
          )}
        </Box>
        ...
        <Box sx={{ width: '100%', height: 280 }}>
          {chartData.length > 0 ? (
              ...
              <PieChart margin={{ top: 25, right: 0, bottom: 0, left: 0 }}>
                ...
                >
                  {chartData.map((entry, index) => (
                    <Cell 
                      key={`cell-${index}`} 
                      fill={projectColorManager.getColorByName(entry.name, projects)}
                    />
                  ))}
                </Pie>
            ...
            <Box sx={{ 
              ...
              <Typography variant="body1" color="text.secondary">
                {isEnglish ? 'No project data for this month' : 'この月のプロジェクトデータがありません'}
              </Typography>
            </Box>
          )}
        </Box>
