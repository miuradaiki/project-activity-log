...
1. `ProjectProgressCard/types.ts` を作成
2. Props型、内部状態型を定義
3. 既存ファイルからインポートするよう変更

#### Step 2-1-2: useProjectProgress フックの抽出

1. テストファイル `useProjectProgress.test.ts` を作成（Red）
2. フックを実装（Green）
3. リファクタリング
