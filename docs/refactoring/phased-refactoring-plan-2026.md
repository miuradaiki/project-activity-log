...
- Electron 29.4.6 → 40.0.0
- electron-builder 24.13.3 → 26.4.0

**結果**:

- 破壊的変更なし（使用中のAPIは全て互換）
- 全テスト通過: 469テスト
- ビルド成功

**⚠️ 重要な教訓: preloadスクリプトのESM移行について**

Electronのpreloadスクリプトは、プロジェクトが`"type": "module"`を使用していても、**CommonJS形式で維持する必要がある**。

| ファイル          | 形式         | 理由                                             |
| ----------------- | ------------ | ------------------------------------------------ |
| `main.js`         | ESM          | メインプロセスはNode.jsのESMを使用               |
| `storageUtils.js` | ESM          | メインプロセスで使用                             |
| `preload.js`      | **CommonJS** | レンダラープロセスのコンテキストで実行されるため |

preloadスクリプトはレンダラープロセスのコンテキストで実行され、メインプロセスの`package.json`設定（`"type": "module"`）とは独立している。ESM形式に変換すると、`contextBridge`や`ipcRenderer`が正しく動作せず、データ読み込みに失敗する。

**将来のElectronアップグレード時の注意**:

1. preloadスクリプトはCommonJS形式を維持すること
2. `require()`構文を`import`に変換しないこと
3. ファイル拡張子を`.mjs`に変更しないこと
